...
#butterworth highpass with cutoff at 180, order of 4
#user bwf for smooth analog rolloff
def butter_bandpass(x, sr, low=250.0, high=3500.0, order=4):
    b, a = sig.butter(order, [low/(0.5*sr), high/(0.5*sr)], btype='bandpass')
...
def extract_notes(wav_file):
    ...
    #apply bwf
    y_hp = butter_bandpass(y_harmonic, sr, low=250.0, high=1000.0, order=4)
    ...
    stft = librosa.stft(y_hp, n_fft=n_fft, hop_length=hop_length) #computes the short term fourier transform
    ...
    for i in range(log_magnitude.shape[1]):
        ...
        peaks, _ = sig.find_peaks(frame, height=np.mean(frame) - 10)
        if len(peaks) > 0:
    ...
    dur_tolerance = 80 #reject any note duration shorter than dur_tolerance ms, can be changed based on bpm
